app.controller("AddDatHangCtrl", function ($scope, $http) {
    ...
    $scope.init = () => {
        localStorage.removeItem('cart_orderProd');
        //Load Branch
    ...
    $scope.cart = {
        ...
        add(id) {
            if (!$scope.selectedOption) {
                this.showAlert('Hãy Chọn Chi Nhánh Đặt Hàng');
            } else {
        ...
        saveToLocalStorage() {
            ...
            localStorage.setItem("cart_orderProd", json);
        },
        //Doc gio hang tu local storage
        loadFromLocalStorage() {
            var json = localStorage.getItem("cart_orderProd");
